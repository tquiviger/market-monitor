...
def get_plan_list(year):
    response = api.call_get(url=base_url + '/plan/year/{0}'.format(year))['data']
    return sorted(response, key=lambda x: x['name'])


def get_country_plan_list(country, year):
    response = api.call_get(url=base_url + '/plan/country/{0}'.format(country))['data']
    return list(filter(lambda x: x['years'][0]['year'] == str(year), response))


def get_wfp_funding():
    response_source = api.call_get(
...
def get_funding_for_orga_and_cluster(organization, cluster):
    ...
    while True:
        ...
        if next_page_url == '':
            ...
            response = api.call_get(url=next_page_url)
...
def get_country_funding_for_year(iso_code, year):
    ...
    for plan in plan_list:
        ...
        required = 0
        percentage = 0
        plan_funding = api.call_get(url=base_url + '/fts/flow?planid={0}&groupby=globalcluster'.format(plan['id']))['data']
